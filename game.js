...
class Ball {
	...
	setupVelocity(){
		...
		this.velocity = new Vector2(velocityX, velocityY).normalize().multiply(SPEED_MULTIPLIER);
	}
	...
	update(players){
		...
		for(const player of players){
			if( Math.abs(nextPos.x - player.pos.x) < 1 + COLLIDER_SIZE_OFFSET && Math.abs(nextPos.y - player.pos.y) <= PLAYER_HEIGHT + COLLIDER_SIZE_OFFSET){
				this.velocity.x = -this.velocity.x;
				nextPos.x = this.pos.x + this.velocity.x;
				this.velocity.y += player.velocity / 4;
				this.velocity.y = myMath.clamp(this.velocity.y, -MAX_VERTICAL_VELOCITY, MAX_VERTICAL_VELOCITY);
				this.increaseSpeed();
			}
		}
		this.pos = nextPos;
	...
	increaseSpeed(){
		...
		this.velocity.x = myMath.clamp(this.velocity.x, -MAX_HORIZONTAL_VELOCITY, MAX_HORIZONTAL_VELOCITY);
	}
}
...
class Game {
	...
	update() {
		...
		for(const player of players){
			player.pos.y = myMath.clamp(player.pos.y, PLAYER_HEIGHT + 1, HEIGHT - PLAYER_HEIGHT - 1);
			player.update();
		}
		this.checkGoal();
		this.sendToClient();
